...
This endpoint includes current Tithi, Nakshatra, and Yoga end times. It does not include Karana or Vara boundary end times, monthly Panchang, muhurat, or interpretation text.
...
The `tithi`, `nakshatra`, and `yoga` sections include `end_time_local` and `end_time_utc` for their next boundaries.
