...
class TithiWithBoundaryResult(TithiResult):
    """Typed output for a Tithi lookup with boundary end time."""
...
def get_tithi_with_boundary(
...
) -> TithiWithBoundaryResult:
    """Return current Tithi with its next boundary end time.
    ...
    Returns:
        TithiWithBoundaryResult containing current Tithi details plus
    ...
    current_tithi = get_tithi(sun_longitude, moon_longitude)
    target_boundary = current_tithi["end_angle"]

    end_datetime_utc = search.find_next_longitude_boundary(
        julian_day_result.utc_datetime,
        _calculate_tithi_angle_at_utc,
        target_boundary,
    )
    ...
    result: TithiWithBoundaryResult = {
        **current_tithi,
...
def _calculate_tithi_angle_at_utc(utc_datetime: datetime) -> float:
