...
def test_calculate_akshavedamsa_position_movable_first_part_starts_mesha() -> None:
    result = varga.calculate_akshavedamsa_position(0.5)

    assert result["varga"] == "D45"
    assert result["varga_number"] == 45
    assert result["varga_name"] == "Akshavedamsa"
    assert result["akshavedamsa_part"] == 1
    assert result["akshavedamsa_rashi"]["sanskrit"] == "Mesha"
    assert result["rashi"]["sanskrit"] == "Mesha"
    assert result["source_rashi"]["sanskrit"] == "Mesha"
    assert result["source_degree"] == 0.5
    assert result["division_index"] == 1
...
def test_calculate_akshavedamsa_position_fixed_first_part_starts_simha() -> None:
    result = varga.calculate_akshavedamsa_position(30.5)

    assert result["akshavedamsa_part"] == 1
    assert result["akshavedamsa_rashi"]["sanskrit"] == "Simha"
    assert result["source_rashi"]["sanskrit"] == "Vrishabha"
    assert result["source_rashi"]["modality"] == "Fixed"
    assert result["source_degree"] == 0.5
    assert result["division_index"] == 1
...
def test_calculate_akshavedamsa_position_dual_first_part_starts_dhanu() -> None:
    result = varga.calculate_akshavedamsa_position(60.5)

    assert result["akshavedamsa_part"] == 1
    assert result["akshavedamsa_rashi"]["sanskrit"] == "Dhanu"
    assert result["source_rashi"]["sanskrit"] == "Mithuna"
    assert result["source_rashi"]["modality"] == "Dual"
    assert result["source_degree"] == 0.5
    assert result["division_index"] == 1
...
def test_calculate_akshavedamsa_position_last_part_counts_from_start() -> None:
    result = varga.calculate_akshavedamsa_position(29.9)

    assert result["akshavedamsa_part"] == 45
    assert result["akshavedamsa_rashi"]["sanskrit"] == "Dhanu"
    assert result["source_rashi"]["sanskrit"] == "Mesha"
    assert result["source_degree"] == 29.9
    assert result["division_index"] == 45


def test_calculate_akshavedamsa_boundary_between_parts() -> None:
    boundary = 30.0 / 45.0
    result = varga.calculate_akshavedamsa_position(boundary)

    assert result["akshavedamsa_part"] == 2
    assert result["akshavedamsa_rashi"]["sanskrit"] == "Vrishabha"
    assert result["source_degree"] == round(boundary, 6)


def test_calculate_akshavedamsa_position_wraps_from_late_rashis() -> None:
    result = varga.calculate_akshavedamsa_position(329.9)
...
def test_calculate_akshavedamsa_position_accepts_planet_shaped_data() -> None:
    result = varga.calculate_akshavedamsa_position(
        {
            "planet": "venus",
            "sidereal_longitude": 60.5,
        }
    )

    assert result["varga_code"] == "D45"
    assert result["source_rashi"]["sanskrit"] == "Mithuna"
    assert result["akshavedamsa_part"] == 1
    assert result["akshavedamsa_rashi"]["sanskrit"] == "Dhanu"


def test_calculate_akshavedamsa_position_rejects_invalid_input() -> None:
    with pytest.raises(TypeError, match="sidereal_longitude must be a real number"):
        varga.calculate_akshavedamsa_position(True)  # type: ignore[arg-type]

    with pytest.raises(ValueError, match="longitude must be finite"):
        varga.calculate_akshavedamsa_position(float("nan"))
